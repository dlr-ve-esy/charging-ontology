...
--output {output}"
    ...
    annotation = "This class was extracted from the EventOntology because the newest BFO does not have it."
    sp.call(extract_call.format(jar= Path(ROBOT_PATH).resolve().as_posix(),
                                input=Path(EVENT_ONTOLOGY).resolve().as_posix(), 
                                method="MIREOT", 
                                lower_term=CLASS_IRI, 
                                upper_term=UPPER_TERM,
                                annotation=annotation,
                                output=Path(TARGET).resolve().as_posix()), shell=True)
else:
